...
const Card: React.FC<{ book: IBook }> = ({
	book: { book_id, title, author, categories, photoUrl },
}) => {
	return (
		<div className='relative bg-white-300 w-full h-fit flex flex-col justify-evenly p-4 border border-gray-200 rounded-lg shadow-md '>
			<div className='flex items-center p-6 gap-6'>
				...
				<div className='w-full flex flex-col items-center text-center gap-2'>
					...
					<div className=' px-6'>
						{categories.map((category) => (
							<Link
								...
								key={category.id}
							>
								<button className='w-fit px-4 py-1 rounded-md text-small border border-white-700 m-1 hover:bg-white-500'>
									{category.name}
								</button>
							</Link>
						))}
					...
					<Link
						className='mt-4 w-full py-2 bg-blue text-white text-semiSmall rounded-md hover:bg-blue-300'
						href={`/library/${book_id}`}
					>
						<button className=''>Leer Prólogo</button>
					</Link>
				</div>
			</div>
		</div>
