...
export default function ProfilePage({ params }: { params: { id: string } }) {
  ...
  const authenticatedUserId = user?.userId; // ID del usuario autenticado
  ...
  const isOwner = String(authenticatedUserId) === params.id;
    ...
    <div className="relative container mx-auto text-center my-10 border-2 rounded-xl">
      <div className="flex items-center justify-between py-16 px-12 border-b-2 bg-white-300">
        <div className="w-32 h-32 relative rounded-full overflow-hidden shadow-md bg-white">
          {userData?.photoUrl && (
            <Image
              src={userData.photoUrl || "https://th.bing.com/th/id/OIP.2KWvyGN-HqjoxRX_UoY0zQHaHa?rs=1&pid=ImgDetMain"}
              alt="Profile Image"
              layout="fill"
              objectFit="cover"
            />
        ...
        <div className="w-1/2 text-left mr-12">
          {isOwner ? (
            ...
            <div>
              <h2 className="text-h2">{userData?.name || "Nombre no disponible"}</h2>
              <p className="text-p">{userData?.description || "Descripción no disponible"}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex rounded-md gap-4">
          <Link
            href={`/profile/${params.id}/books`}
            className="flex flex-col text-semiSmall border-2 border-transparent hover:shadow-md hover:transition-all p-2 rounded-md"
          >
      ...
      <div className="m-12 mb-16">
        <h2 className="text-h2 font-semibold mb-16 text-start border-b-8 inline-block">
          Mis Lecturas
        </h2>
        <ul className="list-disc grid grid-cols-3 gap-12 items-center justify-evenly list-none text-left w-full">
          {books.map((book) => (
            <li
              className="flex flex-col items-center justify-evenly gap-4 text-center w-full h-content rounded-md py-8 bg-white-300"
              key={book.book_id}
            >
              <img
                className="w-40 h-40 border-black object-cover rounded-md"
                src={book.photoUrl}
                alt={book.title}
              />
